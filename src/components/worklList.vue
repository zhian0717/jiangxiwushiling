<template>
    <div class="work-flow-cart-list webpart-global" style="position: relative;
        box-sizing: content-box;overflow: auto;
        height: 100%;">
        <el-tabs class="tabs-header-padding" v-model="activeName">
            <el-tab-pane v-for="item in label" :key="item.active" :label="item.name" :name="item.active">
                <div v-if="showBadge(item.active)" slot="label">
                    {{ item.name }}
                    <span class="bradge" style=" background-color: #f56c6c;
        border-radius: 10px;
        color: #fff;
        ...
        text-align: center;
        white-space: nowrap;
        border: 1px solid #fff;">{{ getWorkCountDesc(item.active) }}</span>
                </div>
            </el-tab-pane>
        </el-tabs>
        <div v-if="showTermSubmitBtn" class="term_submittt_div" style=" cursor: pointer;
        position: absolute;
        top: 5px;
        right: 5px;
        width: 100px;
        ...
        margin-right: 20px;
        margin-top: 5px;
        background-color: #77b8ff;" @click="batchSubmit">批量审批</span>
        </div>
        ...
        <div class="webpart-global__body ContentInfo" style="overflow: auto;
        padding-bottom: 50px;
        top: 84px;">
            <div v-for="(item, index) in infoBox" :key="index" @click="showDataFormView(item)"
                class="Message_content_box_div" style="overflow: hidden;
        padding: 0 0 10px 0;
        border-bottom: 1px solid #eee;">
                <div class="Content_text" style="overflow: hidden;
        margin: 10px 10px 0 10px;">
                    <div class="Message_text" style="overflow: hidden;">
                        <div class="Detailed_information_div" style="overflow: hidden;">
                            <div class="Detailed_information_examples_div" style=" padding: 0;
        cursor: pointer;
        overflow: hidden;
        border-radius: 2px;">
                                <div class="Detailed_information_header" style=" left: 0px;
        top: 0;
        margin: 15px;
        text-align: center;
        border-radius: 50%;
        ...
        overflow: hidden;
        float: left;">
                                    <img style="width: 36px; height: 36px;border-radius: 50%;" :src="getIcon(item)" />
                                </div>
                                <div class="Detailed_information" style="width: auto;
        overflow: hidden;">
                                    <div class="Title" style=" margin: 15px 0 0 0;
        color: #16263c;
        font-size: 14px;
        float: left;
        font-weight: bold;">{{ item.Title }}</div>
                                    <div class="Approval_div" style=" float: right;
        ...
        color: #fff;
        font-weight: lighter;">
                                        <span :class="getColor(item.State)" style="font-size: 12px;
        padding: 2px 4px;
        border-radius: 6px;">{{
                                            getState(item.State)
                                        }}</span>
                                ...
                                <div class="Detailed_information" style="width: auto;
        overflow: hidden;">
                                    <div class="Originator_div" style=" float: left;
        margin: 1px 0 0 0;
        color: #323232;
        font-size: 12px;">
                                    ...
                                    <div class="Dealing_div" style="overflow: hidden;
        color: #757575;
        float: left;
        font-size: 13px;
        margin: 0 0 0 10px;
        padding: 0 0 0 0;">接收时间：{{ item.ReceiveTime }}</div>
                                    <div class="Serial_number" style="float: right;
        margin: 0 15px 0 0;
        color: #323232;
        font-size: 12px;">审批编号：{{ item.SequenceNo }}</div>
                                </div>
                            </div>
                        </div>
                        <div class="Detailed_information_div" style="overflow: hidden;">
                            <div class="Information_operations_div" style="  overflow: hidden;
        margin: 0;">
                                <div class="Information_operations_text" style=" margin-bottom: 0;
        padding-bottom: 0;
        margin: 0 0 0 68px;
        overflow: hidden;
        line-height: 22px;
        ...
        margin: 0 0 0 0;">当前处理人：</label>
            ...
            <div v-if="infoBox.length === 0" class="blank" style="display: flex;
        justify-content: center;
        align-items: center;
        height: 200px;
        font-size: 30px;
        font-weight: bolder;
        color: #e1e1e1;">暂无数据</div>
            <div v-if="showMore" class="load-more" style=" font-size: 14px;" v-loading="LoadingMore" element-loading-spinner="el-icon-loading">
                <div class="load-more-btn" style="border: 1px solid #fff;
        border-radius: 6px;
        padding: 6px;
        width: 100px;
        margin: 5px auto;
        cursor: pointer;
        text-align: center;" @click="getMore(activeName)">查看更多</div>
            </div>
        </div>
    </div>
</template>
