            ...
            <el-row style="padding: 10px 20px" :gutter="10">
                ...
                >
                    ...
                    >
                        <home-tree
                            ref="home-tree"
                            :popover="$refs['card-list-popover']"
                            :data="SearchCondition[0].Value.split(',')"
                            @confirm="confirm"
                        ></home-tree>

                        <el-input
                            slot="reference"
                            ref="search-process-input"
                            v-model="SelectedName"
                            placeholder="搜索流程类别"
                            @change="searchByCondition($event, 0)"
                        >
                            <i
                                slot="suffix"
                                style="cursor: pointer"
                                @click.stop="clearSelectedName"
                ...
                >
                    ...
                    <el-input
                        @input="searchByCondition($event, 1)"
                ...
                >
                    ...
                    <el-date-picker
                        @input="searchByCondition($event, 2)"
                        v-model="StartTime"
                        type="date"
                        placeholder="选择日期"
                    ></el-date-picker>
                ...
                >
                    ...
                    <el-date-picker
                        @input="searchByCondition($event, 3)"
                        v-model="EndTime"
                        type="date"
                        placeholder="选择日期"
                    ></el-date-picker>
...
let { DataFormAction, AppItemType, FormViewType, ShowState, IsNull, Format } = Const;
    ...
    methods: {
        ...
        showDataFormView(item) {
            ...
            let parameters = {
                ...
                cardType: cardType,
                formViewType:
                    this.activeName === 'unFinishWork' ? FormViewType.Edit : FormViewType.View,
            };
        ...
        confirm(data = []) {
            this.SearchCondition[0].Value = data.map(({ Code }) => Code).join(',');
            this.SelectedName = data.map(({ Name }) => Name).join(',');
